...
function addCalendarEventKeys(calendarSync, submissionId) {
  return {
    ...calendarSync,
    submissionId,
    events: calendarSync.events.map((event, index) => ({
      ...event,
      syncEventId: `${submissionId}-${event.type === 'absence' ? submissionKeyPart(event.absenceReason) : 'weekly'}-${event.startDate}-${index + 1}`
    }))
  };
}
...
function createEmailForm() {
  ...
  emailForm.innerHTML = `
    ...
    <input type="hidden" name="gmt_basic_hours" data-clean-field="gmtBasicHours">
    <input type="hidden" name="gmt_ot15_hours" data-clean-field="gmtOt15Hours">
    <input type="hidden" name="gmt_ot20_hours" data-clean-field="gmtOt20Hours">
    <input type="hidden" name="gmt_absence_count" data-clean-field="gmtAbsenceCount">
...
async function submitTimesheet(event) {
  ...
  try {
    ...
    field('gmtOt20Hours').value = String(hours(totals.ot20));
    field('gmtAbsenceCount').value = String(calendarSyncWithIds.events.filter((entry) => entry.type === 'absence').length);
