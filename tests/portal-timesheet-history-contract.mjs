...
assert.doesNotMatch(html, /<iframe[^>]+portal-history-frame|id="portal-history-frame"/, 'timesheet history uses the native interactive calendar and preview');
assert.match(html, /timesheet-calendar-grid/);
assert.match(html, /timesheet-history-preview/);
assert.match(html, /timesheet-history-list/);
assert.match(html, /timesheet-history-preview/);
assert.match(html, /estimate-history-layout/);
...
assert.match(config, /timesheetHistoryScopes:\s*\["https:\/\/service\.flow\.microsoft\.com\/\/\.default"\]/);
